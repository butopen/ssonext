...
type InputStore<T> = Writable<T> | Mergeable<T>;
type Loggable = (<T, S extends InputStore<T>>(store: S) => S) & { enabled: boolean };
...
function update<T>(
  ...
  if (loggable.enabled) {
    ...
    const stackInfo = split[4] || split[3];
    if (stackInfo) {
      const tokens = /(.*)\((.*)\)/gi.exec(stackInfo);
      if (tokens) {
        const functionName = tokens[1];
        const functionLink = tokens[2];
        log(functionName, functionLink, result, ps, ns);
      } else log('', '', result, ps, ns);
    } else {
      log('', '', result, ps, ns);
    }
  }
